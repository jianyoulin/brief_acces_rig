...
# =============================================================================
    ...
    if len(file_check) != 0:
        ...
        detail_df=pd.read_csv(file_check[0], header=0,sep='\t') # get info from datasheet
        print(detail_df.head())
        #detail_df=pd.read_csv(file_check[1], header=0,sep='\t')
        ...
        for case in range(detail_row.shape[1]):
            if detail_df.Notes[detail_row[:,case][0]].lower() in \
                ...
                and detail_df.Animal[detail_row[:,case][0]].lower() in Detail_Dict['Animal'].lower():
